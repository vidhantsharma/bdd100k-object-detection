...
def images_to_coco_format(images: List[Dict[str, Any]], categories: List[str]) -> Dict:
    """
    Convert BDD images with annotations to COCO format.
    
    Args:
        images: List of image dicts (each containing boxes, labels, areas, etc.)
        categories: List of category names (e.g., ['car', 'person', 'truck', ...])
    ...
    Returns:
        COCO format dictionary:
        {
            ...
            'annotations': [
                {'id': 1, 'image_id': 1, 'category_id': 3, 'bbox': [x,y,w,h], ...},
                {'id': 2, 'image_id': 1, 'category_id': 1, 'bbox': [x,y,w,h], ...},
                {'id': 3, 'image_id': 2, 'category_id': 2, 'bbox': [x,y,w,h], ...},
            ],
    ...
    for image_id, img in enumerate(images, start=1):
        coco_format['images'].append({
            'id': image_id,
            'file_name': img['name'],
            'width': 1280,  # BDD100K default
            'height': 720   # BDD100K default
        })
        
        # Add annotations (objects) for this image
        for box, label, area in zip(img['boxes'], img['labels'], img['areas']):
